        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAchievements = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAchievements">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Achievement</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AchievementId" type="text" placeholder="Enter AchievementId" v-model="achievementToAdd.achievementId"></base-input>
  <base-input label="Title" type="text" placeholder="Enter Title" v-model="achievementToAdd.title"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="achievementToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="achievements" :row-key="record => record.AchievementId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AchievementPictureView :achievements="achievements" />
          </div>
...
import AchievementPictureView from './AchievementPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'achievementId',
          visible: true,
          scopedSlots: { customRender: 'achievementId' },
          sorter: true
          //sorter: (a, b) => a.achievementId - b.achievementId,
          //sorter: (a, b) => a.achievementId.localeCompare(b.achievementId),
        },
        {
          title: 'Title',
		dataIndex: 'title',
      ...
      sortBy: 'achievementId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAchievementsTable() {
      ...
      let achievementsTableData = [];
      for (let i = 0; i < this.achievements.length; i++) {
        achievementsTableData.push({
          id: i,
          achievementId: this.achievements[i].achievementId,
          year: this.achievements[i].year,
          date: this.achievements[i].date,
          competitionId: this.achievements[i].competitionId,
          achievementId: this.achievements[i].achievementId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-achievements',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAchievementsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalAchievements = false;

      const currentDate = new Date().getTime();
      this.achievementToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.achievementToAdd);
      console.log(jsonData);
      
      const res = await AchievementService.addAchievement(jsonData);
