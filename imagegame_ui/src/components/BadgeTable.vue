        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBadges = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBadges">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Badge</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BadgeId" type="text" placeholder="Enter BadgeId" v-model="badgeToAdd.badgeId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="badgeToAdd.name"></base-input>
  <base-input label="IconUrl" type="text" placeholder="Enter IconUrl" v-model="badgeToAdd.iconUrl"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="badges" :row-key="record => record.BadgeId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BadgePictureView :badges="badges" />
          </div>
...
import BadgePictureView from './BadgePictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'badgeId',
          visible: true,
          scopedSlots: { customRender: 'badgeId' },
          sorter: true
          //sorter: (a, b) => a.badgeId - b.badgeId,
		...
		dataIndex: 'iconUrl',
          ...
          sorter: true
          //sorter: (a, b) => a.iconUrl - b.iconUrl,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} badges`,
      },
      ...
      sortBy: 'badgeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBadgesTable() {
      ...
      for (let i = 0; i < this.badges.length; i++) {
        badgesTableData.push({
          id: i,
          badgeId: this.badges[i].badgeId,
          year: this.badges[i].year,
          date: this.badges[i].date,
          competitionId: this.badges[i].competitionId,
          badgeId: this.badges[i].badgeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-badges',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBadgesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalBadges = false;

      const currentDate = new Date().getTime();
      this.badgeToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.badgeToAdd);
      console.log(jsonData);
      
      const res = await BadgeService.addBadge(jsonData);
