        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Challenge Completions</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalChallengeCompletions = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalChallengeCompletions">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ChallengeCompletion</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ChallengeCompletionId" type="text" placeholder="Enter ChallengeCompletionId" v-model="challengeCompletionToAdd.challengeCompletionId"></base-input>
  <base-input label="CompletionDate" type="text" placeholder="Enter CompletionDate" v-model="challengeCompletionToAdd.completionDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="challengeCompletions" :row-key="record => record.ChallengeCompletionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ChallengeCompletionPictureView :challengeCompletions="challengeCompletions" />
          </div>
...
import ChallengeCompletionPictureView from './ChallengeCompletionPictureView.vue';


const challengeCompletionsColumns = [
...
export default {
  ...
  components: {
    ...
    Pagination,
    ChallengeCompletionPictureView
  },

  data() {
    return {
      modalChallengeCompletions: false,
        isTableView: true,

      columns: [
        {
          title: 'Challenge Completion Id',
		dataIndex: 'challengeCompletionId',
          visible: true,
          scopedSlots: { customRender: 'challengeCompletionId' },
          sorter: true
          //sorter: (a, b) => a.challengeCompletionId - b.challengeCompletionId,
          //sorter: (a, b) => a.challengeCompletionId.localeCompare(b.challengeCompletionId),
        },
        {
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} challengeCompletions`,
      },

      challengeCompletions: [],
      challengeCompletionToAdd: {},

      challengeCompletionsTable: {
        columns: [...challengeCompletionsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'challengeCompletionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderChallengeCompletionsTable() {
      ...
      let challengeCompletionsTableData = [];
      for (let i = 0; i < this.challengeCompletions.length; i++) {
        challengeCompletionsTableData.push({
          id: i,
          challengeCompletionId: this.challengeCompletions[i].challengeCompletionId,
          year: this.challengeCompletions[i].year,
          date: this.challengeCompletions[i].date,
          competitionId: this.challengeCompletions[i].competitionId,
          challengeCompletionId: this.challengeCompletions[i].challengeCompletionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-challenge-completions',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForChallengeCompletionsChanged', this.searchQuery);
		//this.renderChallengeCompletionsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalChallengeCompletions = false;

      const currentDate = new Date().getTime();
      this.challengeCompletionToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.challengeCompletionToAdd);
      console.log(jsonData);
      
      const res = await ChallengeCompletionService.addChallengeCompletion(jsonData);
