        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalChallenges = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalChallenges">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Challenge</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ChallengeId" type="text" placeholder="Enter ChallengeId" v-model="challengeToAdd.challengeId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="challengeToAdd.description"></base-input>
  <base-input label="Reward" type="text" placeholder="Enter Reward" v-model="challengeToAdd.reward"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="challenges" :row-key="record => record.ChallengeId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ChallengePictureView :challenges="challenges" />
          </div>
...
import ChallengePictureView from './ChallengePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'challengeId',
          visible: true,
          scopedSlots: { customRender: 'challengeId' },
          sorter: true
		...
		dataIndex: 'reward',
          ...
          //sorter: (a, b) => a.reward.localeCompare(b.reward),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} challenges`,
      },
      ...
      sortBy: 'challengeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderChallengesTable() {
      ...
      let challengesTableData = [];
      for (let i = 0; i < this.challenges.length; i++) {
        challengesTableData.push({
          id: i,
          challengeId: this.challenges[i].challengeId,
          year: this.challenges[i].year,
          date: this.challenges[i].date,
          competitionId: this.challenges[i].competitionId,
          challengeId: this.challenges[i].challengeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-challenges',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForChallengesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.challengeToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.challengeToAdd);
      console.log(jsonData);
      
      const res = await ChallengeService.addChallenge(jsonData);
