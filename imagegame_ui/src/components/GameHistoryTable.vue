        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Game Historys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalGameHistorys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalGameHistorys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add GameHistory</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="GameHistoryId" type="text" placeholder="Enter GameHistoryId" v-model="gameHistoryToAdd.gameHistoryId"></base-input>
  <base-input label="Outcome" type="text" placeholder="Enter Outcome" v-model="gameHistoryToAdd.outcome"></base-input>
  <base-input label="Score" type="text" placeholder="Enter Score" v-model="gameHistoryToAdd.score"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="gameHistorys" :row-key="record => record.GameHistoryId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <GameHistoryPictureView :gameHistorys="gameHistorys" />
          </div>
...
import GameHistoryPictureView from './GameHistoryPictureView.vue';


const gameHistorysColumns = [
  "gameHistoryId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    GameHistoryPictureView
  },

  data() {
    return {
      modalGameHistorys: false,
        isTableView: true,

      columns: [
        {
          title: 'Game History Id',
		dataIndex: 'gameHistoryId',
          visible: true,
          scopedSlots: { customRender: 'gameHistoryId' },
          sorter: true
          //sorter: (a, b) => a.gameHistoryId - b.gameHistoryId,
          //sorter: (a, b) => a.gameHistoryId.localeCompare(b.gameHistoryId),
        },
        {
          title: 'Outcome',
		dataIndex: 'outcome',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} gameHistorys`,
      },
      ...
      gameHistorysTable: {
        columns: [...gameHistorysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'gameHistoryId',           // Column to sort by
  ...
  methods: {
    ...
    async renderGameHistorysTable() {
      ...
      let gameHistorysTableData = [];
      for (let i = 0; i < this.gameHistorys.length; i++) {
        gameHistorysTableData.push({
          id: i,
          gameHistoryId: this.gameHistorys[i].gameHistoryId,
          year: this.gameHistorys[i].year,
          date: this.gameHistorys[i].date,
          competitionId: this.gameHistorys[i].competitionId,
          gameHistoryId: this.gameHistorys[i].gameHistoryId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-game-historys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForGameHistorysChanged', this.searchQuery);
		//this.renderGameHistorysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalGameHistorys = false;

      const currentDate = new Date().getTime();
      this.gameHistoryToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.gameHistoryToAdd);
      console.log(jsonData);
      
      const res = await GameHistoryService.addGameHistory(jsonData);
