        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalInAppPurchases = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInAppPurchases">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add InAppPurchase</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="InAppPurchaseId" type="text" placeholder="Enter InAppPurchaseId" v-model="inAppPurchaseToAdd.inAppPurchaseId"></base-input>
  <base-input label="ProductId" type="text" placeholder="Enter ProductId" v-model="inAppPurchaseToAdd.productId"></base-input>
  <base-input label="PurchaseDate" type="text" placeholder="Enter PurchaseDate" v-model="inAppPurchaseToAdd.purchaseDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="inAppPurchases" :row-key="record => record.InAppPurchaseId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InAppPurchasePictureView :inAppPurchases="inAppPurchases" />
          </div>
...
import InAppPurchasePictureView from './InAppPurchasePictureView.vue';


const inAppPurchasesColumns = [
  "inAppPurchaseId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    InAppPurchasePictureView
  },

  data() {
    return {
      modalInAppPurchases: false,
        isTableView: true,

      columns: [
        {
          title: 'In App Purchase Id',
		dataIndex: 'inAppPurchaseId',
          visible: true,
          scopedSlots: { customRender: 'inAppPurchaseId' },
          sorter: true
          //sorter: (a, b) => a.inAppPurchaseId - b.inAppPurchaseId,
          //sorter: (a, b) => a.inAppPurchaseId.localeCompare(b.inAppPurchaseId),
        },
        {
          title: 'Product Id',
		dataIndex: 'productId',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} inAppPurchases`,
      },
      ...
      inAppPurchasesTable: {
        columns: [...inAppPurchasesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'inAppPurchaseId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInAppPurchasesTable() {
      ...
      for (let i = 0; i < this.inAppPurchases.length; i++) {
        inAppPurchasesTableData.push({
          id: i,
          inAppPurchaseId: this.inAppPurchases[i].inAppPurchaseId,
          year: this.inAppPurchases[i].year,
          date: this.inAppPurchases[i].date,
          competitionId: this.inAppPurchases[i].competitionId,
          inAppPurchaseId: this.inAppPurchases[i].inAppPurchaseId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-in-app-purchases',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInAppPurchasesChanged', this.searchQuery);
		//this.renderInAppPurchasesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalInAppPurchases = false;

      const currentDate = new Date().getTime();
      this.inAppPurchaseToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.inAppPurchaseToAdd);
      console.log(jsonData);
      
      const res = await InAppPurchaseService.addInAppPurchase(jsonData);
