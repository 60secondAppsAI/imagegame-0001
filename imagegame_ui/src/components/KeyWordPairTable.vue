        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Key Word Pairs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalKeyWordPairs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalKeyWordPairs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add KeyWordPair</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="KeyWordPairId" type="text" placeholder="Enter KeyWordPairId" v-model="keyWordPairToAdd.keyWordPairId"></base-input>
  <base-input label="WordOne" type="text" placeholder="Enter WordOne" v-model="keyWordPairToAdd.wordOne"></base-input>
  <base-input label="WordTwo" type="text" placeholder="Enter WordTwo" v-model="keyWordPairToAdd.wordTwo"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="keyWordPairs" :row-key="record => record.KeyWordPairId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <KeyWordPairPictureView :keyWordPairs="keyWordPairs" />
          </div>
...
import KeyWordPairPictureView from './KeyWordPairPictureView.vue';


const keyWordPairsColumns = [
  "keyWordPairId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    KeyWordPairPictureView
  },

  data() {
    return {
      modalKeyWordPairs: false,
        isTableView: true,

      columns: [
        {
          title: 'Key Word Pair Id',
		dataIndex: 'keyWordPairId',
          visible: true,
          scopedSlots: { customRender: 'keyWordPairId' },
          sorter: true
          //sorter: (a, b) => a.keyWordPairId - b.keyWordPairId,
          //sorter: (a, b) => a.keyWordPairId.localeCompare(b.keyWordPairId),
        },
        {
          title: 'Word One',
		dataIndex: 'wordOne',
          visible: true,
          scopedSlots: { customRender: 'wordOne' },
          sorter: true
          //sorter: (a, b) => a.wordOne - b.wordOne,
          //sorter: (a, b) => a.wordOne.localeCompare(b.wordOne),
        },
        {
          title: 'Word Two',
		dataIndex: 'wordTwo',
          visible: true,
          scopedSlots: { customRender: 'wordTwo' },
          sorter: true
          //sorter: (a, b) => a.wordTwo - b.wordTwo,
          //sorter: (a, b) => a.wordTwo.localeCompare(b.wordTwo),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} keyWordPairs`,
      },
      ...
      keyWordPairsTable: {
        columns: [...keyWordPairsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'keyWordPairId',           // Column to sort by
  ...
  methods: {
    ...
    async renderKeyWordPairsTable() {
      ...
      let keyWordPairsTableData = [];
      for (let i = 0; i < this.keyWordPairs.length; i++) {
        keyWordPairsTableData.push({
          id: i,
          keyWordPairId: this.keyWordPairs[i].keyWordPairId,
          year: this.keyWordPairs[i].year,
          date: this.keyWordPairs[i].date,
          competitionId: this.keyWordPairs[i].competitionId,
          keyWordPairId: this.keyWordPairs[i].keyWordPairId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-key-word-pairs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForKeyWordPairsChanged', this.searchQuery);
		//this.renderKeyWordPairsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalKeyWordPairs = false;

      const currentDate = new Date().getTime();
      this.keyWordPairToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.keyWordPairToAdd);
      console.log(jsonData);
      
      const res = await KeyWordPairService.addKeyWordPair(jsonData);
