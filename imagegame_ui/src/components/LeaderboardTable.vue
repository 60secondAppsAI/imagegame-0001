        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalLeaderboards = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLeaderboards">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Leaderboard</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LeaderboardId" type="text" placeholder="Enter LeaderboardId" v-model="leaderboardToAdd.leaderboardId"></base-input>
  <base-input label="Rank" type="text" placeholder="Enter Rank" v-model="leaderboardToAdd.rank"></base-input>
  <base-input label="HighScore" type="text" placeholder="Enter HighScore" v-model="leaderboardToAdd.highScore"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="leaderboards" :row-key="record => record.LeaderboardId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LeaderboardPictureView :leaderboards="leaderboards" />
          </div>
...
import LeaderboardPictureView from './LeaderboardPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    LeaderboardPictureView
  },

  data() {
    return {
      modalLeaderboards: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'leaderboardId',
          visible: true,
          scopedSlots: { customRender: 'leaderboardId' },
          sorter: true
          //sorter: (a, b) => a.leaderboardId - b.leaderboardId,
          //sorter: (a, b) => a.leaderboardId.localeCompare(b.leaderboardId),
        },
        {
          title: 'Rank',
		dataIndex: 'rank',
		...
		dataIndex: 'highScore',
          visible: true,
          scopedSlots: { customRender: 'highScore' },
          sorter: true
          //sorter: (a, b) => a.highScore - b.highScore,
          //sorter: (a, b) => a.highScore.localeCompare(b.highScore),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} leaderboards`,
      },
      ...
      sortBy: 'leaderboardId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLeaderboardsTable() {
      ...
      for (let i = 0; i < this.leaderboards.length; i++) {
        leaderboardsTableData.push({
          id: i,
          leaderboardId: this.leaderboards[i].leaderboardId,
          year: this.leaderboards[i].year,
          date: this.leaderboards[i].date,
          competitionId: this.leaderboards[i].competitionId,
          leaderboardId: this.leaderboards[i].leaderboardId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-leaderboards',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLeaderboardsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalLeaderboards = false;

      const currentDate = new Date().getTime();
      this.leaderboardToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.leaderboardToAdd);
      console.log(jsonData);
      
      const res = await LeaderboardService.addLeaderboard(jsonData);
