        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Player Inventorys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPlayerInventorys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPlayerInventorys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PlayerInventory</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PlayerInventoryId" type="text" placeholder="Enter PlayerInventoryId" v-model="playerInventoryToAdd.playerInventoryId"></base-input>
  <base-input label="Quantity" type="text" placeholder="Enter Quantity" v-model="playerInventoryToAdd.quantity"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="playerInventorys" :row-key="record => record.PlayerInventoryId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PlayerInventoryPictureView :playerInventorys="playerInventorys" />
          </div>
...
import PlayerInventoryPictureView from './PlayerInventoryPictureView.vue';


const playerInventorysColumns = [
  "playerInventoryId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    PlayerInventoryPictureView
  },

  data() {
    return {
      modalPlayerInventorys: false,
        isTableView: true,

      columns: [
        {
          title: 'Player Inventory Id',
		dataIndex: 'playerInventoryId',
          visible: true,
          scopedSlots: { customRender: 'playerInventoryId' },
          sorter: true
          //sorter: (a, b) => a.playerInventoryId - b.playerInventoryId,
          //sorter: (a, b) => a.playerInventoryId.localeCompare(b.playerInventoryId),
        },
        {
          title: 'Quantity',
		dataIndex: 'quantity',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} playerInventorys`,
      },

      playerInventorys: [],
      playerInventoryToAdd: {},

      playerInventorysTable: {
        columns: [...playerInventorysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'playerInventoryId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPlayerInventorysTable() {
      ...
      let playerInventorysTableData = [];
      for (let i = 0; i < this.playerInventorys.length; i++) {
        playerInventorysTableData.push({
          id: i,
          playerInventoryId: this.playerInventorys[i].playerInventoryId,
          year: this.playerInventorys[i].year,
          date: this.playerInventorys[i].date,
          competitionId: this.playerInventorys[i].competitionId,
          playerInventoryId: this.playerInventorys[i].playerInventoryId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-player-inventorys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPlayerInventorysChanged', this.searchQuery);
		//this.renderPlayerInventorysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPlayerInventorys = false;

      const currentDate = new Date().getTime();
      this.playerInventoryToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.playerInventoryToAdd);
      console.log(jsonData);
      
      const res = await PlayerInventoryService.addPlayerInventory(jsonData);
