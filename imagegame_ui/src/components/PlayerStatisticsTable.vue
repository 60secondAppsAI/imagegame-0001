...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Player Statisticss</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPlayerStatisticss = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPlayerStatisticss">
                <template slot="header">
  ...
  <base-input label="TotalScore" type="text" placeholder="Enter TotalScore" v-model="playerStatisticsToAdd.totalScore"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="playerStatisticss" :row-key="record => record.PlayerStatisticsId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PlayerStatisticsPictureView :playerStatisticss="playerStatisticss" />
          </div>
...
import PlayerStatisticsPictureView from './PlayerStatisticsPictureView.vue';


const playerStatisticssColumns = [
  "playerStatisticsId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    PlayerStatisticsPictureView
  },

  data() {
    return {
      modalPlayerStatisticss: false,
        isTableView: true,

      columns: [
        {
          title: 'Player Statistics Id',
		dataIndex: 'playerStatisticsId',
          visible: true,
          scopedSlots: { customRender: 'playerStatisticsId' },
          sorter: true
          //sorter: (a, b) => a.playerStatisticsId - b.playerStatisticsId,
          //sorter: (a, b) => a.playerStatisticsId.localeCompare(b.playerStatisticsId),
        },
        {
          title: 'Games Played',
		dataIndex: 'gamesPlayed',
          visible: true,
          scopedSlots: { customRender: 'gamesPlayed' },
          sorter: true
          //sorter: (a, b) => a.gamesPlayed - b.gamesPlayed,
          //sorter: (a, b) => a.gamesPlayed.localeCompare(b.gamesPlayed),
        },
        {
          title: 'Games Won',
		dataIndex: 'gamesWon',
          visible: true,
          scopedSlots: { customRender: 'gamesWon' },
          sorter: true
          //sorter: (a, b) => a.gamesWon - b.gamesWon,
          //sorter: (a, b) => a.gamesWon.localeCompare(b.gamesWon),
        },
        {
          title: 'Total Score',
		dataIndex: 'totalScore',
          ...
          //sorter: (a, b) => a.totalScore.localeCompare(b.totalScore),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} playerStatisticss`,
      },

      playerStatisticss: [],
      playerStatisticsToAdd: {},

      playerStatisticssTable: {
        columns: [...playerStatisticssColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'playerStatisticsId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPlayerStatisticssTable() {
      ...
      let playerStatisticssTableData = [];
      for (let i = 0; i < this.playerStatisticss.length; i++) {
        playerStatisticssTableData.push({
          id: i,
          playerStatisticsId: this.playerStatisticss[i].playerStatisticsId,
          year: this.playerStatisticss[i].year,
          date: this.playerStatisticss[i].date,
          competitionId: this.playerStatisticss[i].competitionId,
          playerStatisticsId: this.playerStatisticss[i].playerStatisticsId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-player-statisticss',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPlayerStatisticssChanged', this.searchQuery);
		//this.renderPlayerStatisticssTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.playerStatisticsToAdd.created = currentDate;
