...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPlayers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPlayers">
                <template slot="header">
  ...
  <base-input label="Email" type="text" placeholder="Enter Email" v-model="playerToAdd.email"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="players" :row-key="record => record.PlayerId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PlayerPictureView :players="players" />
          </div>
...
import PlayerPictureView from './PlayerPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'playerId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPlayersTable() {
      ...
      let playersTableData = [];
      for (let i = 0; i < this.players.length; i++) {
        playersTableData.push({
          id: i,
          playerId: this.players[i].playerId,
          year: this.players[i].year,
          date: this.players[i].date,
          competitionId: this.players[i].competitionId,
          playerId: this.players[i].playerId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-players',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPlayersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.playerToAdd.created = currentDate;
