        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">User Badges</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalUserBadges = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalUserBadges">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add UserBadge</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="UserBadgeId" type="text" placeholder="Enter UserBadgeId" v-model="userBadgeToAdd.userBadgeId"></base-input>
  <base-input label="AwardedDate" type="text" placeholder="Enter AwardedDate" v-model="userBadgeToAdd.awardedDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="userBadges" :row-key="record => record.UserBadgeId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <UserBadgePictureView :userBadges="userBadges" />
          </div>
...
import UserBadgePictureView from './UserBadgePictureView.vue';


const userBadgesColumns = [
  "userBadgeId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    UserBadgePictureView
  },

  data() {
    return {
      modalUserBadges: false,
        isTableView: true,

      columns: [
        {
          title: 'User Badge Id',
		dataIndex: 'userBadgeId',
          visible: true,
          scopedSlots: { customRender: 'userBadgeId' },
          sorter: true
          //sorter: (a, b) => a.userBadgeId - b.userBadgeId,
          //sorter: (a, b) => a.userBadgeId.localeCompare(b.userBadgeId),
        },
        {
          title: 'Awarded Date',
		dataIndex: 'awardedDate',
          visible: true,
          scopedSlots: { customRender: 'awardedDate' },
          sorter: true
          //sorter: (a, b) => a.awardedDate - b.awardedDate,
          //sorter: (a, b) => a.awardedDate.localeCompare(b.awardedDate),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} userBadges`,
      },
      ...
      sortBy: 'userBadgeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderUserBadgesTable() {
      ...
      let userBadgesTableData = [];
      for (let i = 0; i < this.userBadges.length; i++) {
        userBadgesTableData.push({
          id: i,
          userBadgeId: this.userBadges[i].userBadgeId,
          year: this.userBadges[i].year,
          date: this.userBadges[i].date,
          competitionId: this.userBadges[i].competitionId,
          userBadgeId: this.userBadges[i].userBadgeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-user-badges',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForUserBadgesChanged', this.searchQuery);
		//this.renderUserBadgesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalUserBadges = false;

      const currentDate = new Date().getTime();
      this.userBadgeToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.userBadgeToAdd);
      console.log(jsonData);
      
      const res = await UserBadgeService.addUserBadge(jsonData);
