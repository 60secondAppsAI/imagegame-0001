					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateGameSession()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2GameSessions = true">Add</base-button>
								              <modal :show.sync="modal2GameSessions">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Game Session Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="GameSessionId" type="text" placeholder="Enter GameSessionId" v-model="gameSessionDetails.gameSessionId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Player Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PlayerId" type="text" placeholder="Enter PlayerId" v-model="gameSessionDetails.playerId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Start Time</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="gameSessionDetails.startTime"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">End Time</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="gameSessionDetails.endTime"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      gameSessionToAdd: {},
      modal2GameSessions: false,
      gameSessionDetails: null,
  ...
  methods: {
    ...
    async updateGameSession() {
      ...
      const jsonData = JSON.stringify(this.gameSessionDetails);
      const res = await GameSessionService.update(jsonData);
...
//        this.getAllGameSessions();
    ...
    async getGameSessionDetails() {
      try {
			let response = await GameSessionService.get(this.gameSessionId);
			this.gameSessionDetails = response.data;
