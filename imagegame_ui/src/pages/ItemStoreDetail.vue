					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateItemStore()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2ItemStores = true">Add</base-button>
								              <modal :show.sync="modal2ItemStores">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Item Store Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ItemStoreId" type="text" placeholder="Enter ItemStoreId" v-model="itemStoreDetails.itemStoreId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Item Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ItemName" type="text" placeholder="Enter ItemName" v-model="itemStoreDetails.itemName"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Item Price</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ItemPrice" type="text" placeholder="Enter ItemPrice" v-model="itemStoreDetails.itemPrice"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateItemStore() {
      ...
      const jsonData = JSON.stringify(this.itemStoreDetails);
      const res = await ItemStoreService.update(jsonData);
...
//        this.getAllItemStores();
    ...
    async getItemStoreDetails() {
      try {
			let response = await ItemStoreService.get(this.itemStoreId);
			this.itemStoreDetails = response.data;
