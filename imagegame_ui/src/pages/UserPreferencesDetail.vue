					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateUserPreferences()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2UserPreferencess = true">Add</base-button>
								              <modal :show.sync="modal2UserPreferencess">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">User Preferences Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="UserPreferencesId" type="text" placeholder="Enter UserPreferencesId" v-model="userPreferencesDetails.userPreferencesId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Preferred Theme</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PreferredTheme" type="text" placeholder="Enter PreferredTheme" v-model="userPreferencesDetails.preferredTheme"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Language</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Language" type="text" placeholder="Enter Language" v-model="userPreferencesDetails.language"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2UserPreferencess = false;
    ...
    async updateUserPreferences() {
      ...
      const jsonData = JSON.stringify(this.userPreferencesDetails);
      const res = await UserPreferencesService.update(jsonData);
...
//        this.getAllUserPreferencess();
    ...
    async getUserPreferencesDetails() {
      try {
			let response = await UserPreferencesService.get(this.userPreferencesId);
			this.userPreferencesDetails = response.data;
 ...
 formatDate(dateString) {
  ...
  mounted() {
    this.getUserPreferencesDetails();
  },
  computed: {
  },
